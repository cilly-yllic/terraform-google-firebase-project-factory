...
| <a name="input_service_accounts"></a> [service\_accounts](#input\_service\_accounts) | Firebase project Service Accounts list. | <pre>list(object({<br>    account_id   = string<br>    display_name = optional(string)<br>    type         = string # deploy<br>    rules        = optional(list(string), [])<br>    args         = optional(any, {})<br>  }))</pre> | `[]` | no |
